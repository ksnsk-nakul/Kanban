            ...
            <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-4">
                ...
                <div>
                    ...
                    <input name="name" value="{{ old('name', $user->name) }}"
                           class="mt-1 w-full rounded-2xl bg-black/10 dark:bg-black/30 border border-white/10 px-4 py-2 text-sm outline-none focus:border-white/20">
                </div>

                <div>
                    <label class="text-xs text-zinc-400">{{ __('Email') }}</label>
                    <input value="{{ $user->email }}" disabled
                           class="mt-1 w-full rounded-2xl bg-black/5 dark:bg-black/30 border border-white/10 px-4 py-2 text-sm text-zinc-500">
                </div>
            ...
            <div class="glass rounded-3xl p-6">
                ...
                <div class="mt-4 space-y-2">
                    <form method="post" action="{{ route('preferences.locale') }}">
                        ...
                        <select name="locale" class="mt-1 w-full rounded-2xl bg-black/10 dark:bg-black/30 border border-white/10 px-4 py-2 text-sm">
